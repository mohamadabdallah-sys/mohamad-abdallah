...
BOOK, ANSWERS = "كتاب-رياضيات-الصف-السابع.pdf", "دليل-المعلم-الصف-السابع.pdf"
GREEN, GOLD = (0.055, 0.353, 0.204), (0.914, 0.706, 0.298)
HEX = {"teal": "#0E7C7B", "saffron": "#B7791F", "plum": "#7A3E8E", "blue": "#2B5FB3", "green": "#2F7D46", "coral": "#C2452F"}
import content as _c
LESSON_COLOR = {L["id"]: HEX[L["color"]] for L in _c.LESSONS}
LESSON_COLOR.update({f'unit-{U["num"]}': HEX[U["color"]] for U in _c.UNITS})
...
def section_color(keys_by_page, n_pages):
    ...
    starts = sorted((p, k) for k, p in keys_by_page.items() if k in LESSON_COLOR or k in ("cards", "exams", "summary", "howto", "index", "diag"))
    out, cur = [], GREEN
...
def main():
    ...
    for U in content.UNITS:
        ...
        for L in [L_ for L_ in content.LESSONS if L_["id"] in U["lessons"]]:
            titles.append((2, f'الدرس {L["num"]}: {L["title"]}', L["id"]))
            titles += [(3, p, f'{L["id"]}-{i + 1}') for i, p in enumerate(__import__("build").PARTS)]
